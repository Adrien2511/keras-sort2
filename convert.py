...
def main2(args):
    ...
    for section in cfg_pars.sections():                  #pour chaque bloc du fichier yolov3.cfg
        ...
        if section.startswith('convolutional'):            #rentre quand c'est un block de convolution
            ...
            if len(couches) > 1:   #si le nombre d'élément dans la section route est supérieur à 1
                ...
                prev_couche = concatenate_couche      # enregistre la valeur comme derniére couche
            else:                                        # si le route est composé d'un seul élément
                skip_couche = couches[0]                 # enregistre la valeur de la couche à l'indice de l'élément donner par route
